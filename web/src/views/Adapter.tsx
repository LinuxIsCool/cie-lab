...
const BUILD: BuildStep[] = [
  {
    t: "A CIE ToolImpl, modeled on polis.rs",
    d: "Comhairle already carries a tool that wraps Pol.is. A CIE tool follows the same template — CieToolConfig, CieToolSetup, CieReport — so CIE's bridging registers as a first-class Comhairle tool alongside the ones already there.",
    tag: "api/src/tools/cie.rs",
  },
  {
    t: "A cie_statement_aux table + migration",
    d: "Pol.is keeps its per-statement state in polis_statement_aux. CIE gets its own parallel table for the statements, groups, and provenance it mirrors in, added through Comhairle's numbered SQL migration flow.",
    tag: "new migration",
  },
  {
    t: "The enum + match-arm + router surgery",
    d: "Comhairle names every tool in the ToolConfig, ToolSetup, and ReportConfig enums and routes each through router(). A CIE arm joins each one; the Rust compiler enforces exhaustiveness, so the tool is wired everywhere or the build stops.",
    tag: "compiler-enforced",
  },
  {
    t: "A real sync_data implementation",
    d: "The heart of the tool: sync_data mirrors a CIE export into cie_statement_aux — open text becomes Statements, opinion groups become Groups with algorithm provenance — so a Comhairle workflow reads CIE's results as native data.",
    tag: "the live seam",
  },
];
...
const GIFTS = [
  ...
  },
  {
    ...
    d: "CIE's honesty layer distinguishes content hidden for tone from content excluded for integrity, and records which algorithm produced each result. A shared moderation-and-provenance entity gives that distinction a home in Comhairle's model, so the statistics stay trustworthy across the handoff.",
  },
];
    ...
    <div className="max-w-5xl">
      <header className="mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-semibold px-2 py-0.5 rounded bg-slate-800 text-white">P10 · POC #11</span>
        </div>
        <p className="text-sm text-slate-500">CIE's bridging as a native Comhairle tool — the deep-integration "join the crew" path</p>
      </header>
      ...
      <div className="mb-5 space-y-2">
        <Commentary kind="perspective" title="What adapter-as-tool means">
          ...
          Comhairle workflow. An adapter-as-tool makes CIE another such tool: CIE's bridging lives <em>inside</em> a
        ...
        <Commentary kind="choice" title="Inside, where P1 goes alongside">
          ...
          places CIE within Comhairle's own tool spine, so a Comhairle operator selects "CIE" the way they select Pol.is.
          Both are contribution paths; this is the deeper one.
        </Commentary>
        <Commentary kind="goal" title="The white-label angle">
          Once CIE is a Comhairle tool, it travels with every Comhairle instance. Anyone running Comhairle can offer CIE's
          bridging under their own brand — CIE's method reaching communities through hosts CIE never has to run itself.
        </Commentary>
      </div>
      ...
      <div className="mb-5 space-y-2">
        <Commentary kind="methodology" title="Why it waits">
          A tool earns its keep when a workflow reads it. Comhairle's interchange ingest is unwired today, so a CIE tool
          would mirror data into cie_statement_aux with no live consumer yet. The value arrives once Comhairle wires that
          ingest and a Comhairle workflow has a reason to read CIE from within.
        </Commentary>
        <Commentary kind="architecture" title="The license shapes the sequence">
          Comhairle is AGPL-3.0, and its §13 reaches network use: a hosted, modified Comhairle offers its complete source —
          the CIE tool and every change included — to everyone who uses it. That makes a fork a maintained, open commitment.
          So the reversible move leads: export-first through <strong>P1</strong> now, adapter-later here, with CIE's own
      ...
      <Commentary kind="principle" title="Turning compare into contribute">
        ...
        move — improving the shared grammar for every tool it carries, CIE among them, and opening the door the adapter walks
        through later.
      </Commentary>
